...
void Main_Thread(void *argument)
{
    ...
    g_xQueueStrInput = xQueueCreateSet(READCQueueLenght + REDHT11QueueLenght);
    ...
    while (1)
    {
        ...
        if (1)
        {
            ...
            Mp3_data.volume = 0;
            Mp3_data.voice = 0;
            xQueueSend(MP3_Queue, &Mp3_data, 0); // 发送MP3数据
        }

        IntupQueue = xQueueSelectFromSet(g_xQueueStrInput, 0);
